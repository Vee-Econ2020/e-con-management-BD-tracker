...
export default function Slide21_2_2({ fy = "FY2027" }: { fy?: string }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/admin/slides/order-backlog?region=Japan&fy=${fy}`);
            ...
            console.error('Failed to fetch Japan order backlog data:', err);
    ...
    return <OrderBacklogChart data={data} title="Order Backlog Japan" />;
}
