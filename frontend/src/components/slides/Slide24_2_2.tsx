...
export default function Slide24_2_2({ fy = "FY2027" }: { fy?: string }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const response = await fetch(`/api/admin/slides/order-backlog?region=KANZ&fy=${fy}`);
            ...
            console.error('Failed to fetch KANZ order backlog data:', err);
    ...
    return <OrderBacklogChart data={data} title="Order Backlog KANZ" />;
}
