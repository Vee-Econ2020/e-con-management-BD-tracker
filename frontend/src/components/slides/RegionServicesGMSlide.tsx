...
export default function RegionServicesGMSlide({ region, fy = "FY2027" }: { region: string; fy?: string }) {
        ...
        (async () => {
            try {
                ...
                const response = await fetch(
                    `/api/admin/slides/region-services-gross-margin?region=${encodeURIComponent(region)}&fy=${fy}`
                );
                ...
                console.error(`Failed to fetch services GM data for ${region}:`, err);
        ...
        return [
            {
                ...
                textfont: { size: 14, color: '#4a2175', weight: 'bold' } as any,
                cliponaxis: false,
            ...
            },
            {
                ...
                textposition: 'outside' as const,
                textfont: { size: 14, color: '#0e7c7b', weight: 'bold' } as any,
                cliponaxis: false,
    ...
    if (loading) {
        return (
            <div style={{ backgroundColor: '#ffffff', height: '100%', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: '#475569' }}>Loading services gross margin…</div>
            </div>
        );
    }
    if (error || !data) {
        return (
            <div style={{ backgroundColor: '#fef2f2', height: '100%', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
                <div style={{ textAlign: 'center', color: '#991b1b' }}>
                    <div style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '0.5rem' }}>Failed to load services gross margin</div>
                    <div>{error}</div>
                </div>
            </div>
        );
